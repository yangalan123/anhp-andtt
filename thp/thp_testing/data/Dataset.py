...
def prepare_dataloader(args):
    ...
    train_data, num_types = load_data(os.path.join(args.PathDomain, 'train.pkl'), 'train')
    print('[Info] Loading dev data...')
    dev_data, _ = load_data(os.path.join(args.PathDomain, 'dev.pkl'), 'dev')
    print('[Info] Loading test data...')
    test_data, _ = load_data(os.path.join(args.PathDomain, 'test.pkl'), 'test')

    trainloader = get_dataloader(train_data, args.BatchSize, shuffle=True)
    devloader = get_dataloader(dev_data, args.BatchSize, shuffle=False)
    testloader = get_dataloader(test_data, args.BatchSize, shuffle=False)
