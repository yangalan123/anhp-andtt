...
class EventSampler(nn.Module):
    ...
    def draw_next_time_ordinary(self, arguments, fastapprox=True):
        ...
        # in xfmr-nhp, no need to downsample event types since all intensities computed using FF(hidden)
        # # 1 * len(types)
    ...
    def draw_next_time_unbiased(self, arguments):
        ...
        while target_num_samples > 0 and patience > 0:
            ...
            # if min(sampled_times[i, :]) already larger than boundary, no need to consider that row
